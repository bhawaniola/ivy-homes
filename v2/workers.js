...
async function fetchNames(prefix, proxy, groupChar) {
  ...
  try {
    ...
    console.log(
      `[${prefix}] Fetched ${names.length} names | Total: ${allNames.size} | Requests: ${totalRequests} | Proxy: ${proxy}`
    );
    ...
    console.error(`⚠️ Error on prefix "${prefix}" using proxy ${proxy}: ${error.message} | Retrying in ${RETRY_DELAY / 1000}s...`);
